...
## UPDATE 21 — LOST WAKEUP LOCALIZED to the psynch MUTEX drop matching (NOT the condvar, NOT the microthread-resume layer)

Instrumented the full psynch path (kern_synch.c: _psynch_cvwait entry/block, __psynch_cvsignal diff-gate + ksyn_cvsignal wake result, _psynch_mutexwait entry/prepost/block, _psynch_mutexdrop_internal entry + intr-post, ksyn_signal psynch_wait_wakeup return) AND the microthread layer (thread.c thread_unblock; thread.cpp Thread::resume). Ran on the signal-free repro cvstorm2 -DNCONS=1 -DNO_STORM. Findings at the freeze:

- Steady state is a clean ping-pong on ONE mutex (0x100008068) + ONE condvar (0x1000080a8) between the 2 worker microthreads, seq advancing by PTHRW_INC (0x100) each iteration. cvsignal is always woke=1; KSYN_SIGNAL psynch_wait_wakeup always returns 0 (SUCCESS), NEVER 48 (NOT_WAITING). CV side is healthy.
- The freeze is on the MUTEX. Last worker events: `MTX WAIT-block tid=B lseq=0xNN800` (a worker blocks on the mutex) immediately followed by `MTX DROP-enter tid=A ugen=0xNN800 nextgen=0xNN900 inq=1 firstwr=0xNN800` (the peer unlocks, SEES the waiter, inq=1) — and then NOTHING. No `UNBLOCK nstid=<B>`, no further worker op. The blocked worker is never woken; at shutdown it is resumed (proving it was asleep the whole time).
- RESUME layer EXONERATED: `A0 RESUME DROPPED=0` (a wake is never lost because the target is neither running nor suspended); worker UNBLOCKs fire normally (2026 + 1370 across the run). The microthread suspend/resume `_resumePermit` machinery is NOT the bug.
- => the lost wakeup is INSIDE `_psynch_mutexdrop_internal` (kern_synch.c ~597-645, non-firstfit branch): the unlocker computes the grant target `nextgen = ugen + PTHRW_INC`, then `low_writer == nextgen`? / `is_seqhigher(low_writer, nextgen)`? / else `ksyn_queue_find_seq(nextgen)`. In the freeze the single queued waiter's seq does NOT match `nextgen`, `find_seq` returns NULL, so it takes `prepost=true` — marks a preposted wakeup and wakes NOBODY, even though `inq=1` and the waiter is sitting right there. The waiter then waits forever; the unlocker returns to guest and eventually also blocks -> total stall (matches guest: work>0, both threads parked).
- MTX DROP-intrpost=0 and MTX WAIT-recovered-intr=0 across the whole run: the kw_intr "interrupted wakeup" recovery machinery is NOT involved. This is a pure seq-matching miss in the normal grant path.

STATUS: root cause layer PINNED — Apple psynch mutex-drop waiter-seq matching under duct-tape mis-selects the grant target for the actually-queued waiter and preposts instead of waking it (a genuine lost wakeup; deterministic; no signals). The `firstwr`(=ksynq_firstnum) snapshot in the trace is masked differently from the compare and can't by itself tell the failing iteration from the working ones (all 2703 inq=1 drops showed firstwr<nextgen, yet 3396 worker unblocks fired) — so the NEXT step is to trace INSIDE _psynch_mutexdrop_internal the exact branch taken (line604 signal / line614 prepost / line619 find_seq hit/miss) and the queued kwe's kwe_lockseq vs nextgen for the FAILING drop, to nail whether the bug is (a) an off-by-one in nextgen vs the waiter's registered lockseq, (b) is_seqhigher/is_seqlower wraparound at the 0x…00 boundary, or (c) the waiter enqueuing at the wrong seq in ksyn_wait. Then fix + gate boot -> cvstorm2 all-variants RESULT=OK 0 hang -> brew A/B 0 hang. Prod baseline 835946f9, doctor GREEN. Trace lives on branch as a diagnostic commit; repro tmp/cvstorm2.c, harness tmp/a0_cvtrace.sh.

## UPDATE 22 — ROOT CAUSE PINNED TO ONE LINE: Thread::doWork() drops a scheduled wake when the target is still _running (thread.cpp:488)

CORRECTS UPDATE 21's "mutex seq-matching" guess. Traced the wake all the way down through every layer with the firstfit branch instrumented (the mutex is firstfit, so the real path is _psynch_mutexdrop_internal's firstfit branch, kern_synch.c ~572+: inq==0 -> prepost, inq>0 -> ksyn_mtxsignal(first waiter)). ksyn_mtxsignal_ret was ALWAYS 0/SUCCESS; DROP-intrpost / DROP-MISS / find_seq NEVER involved; CV side clean; RESUME DROPPED=0. So NOT the psynch protocol, NOT the seq-matching, NOT the resume permit.

The lost wake is in the WORK-QUEUE DISPATCH. Instrumented Thread::doWork()'s early-return guards. At the freeze, exactly ONE anomaly in the whole run:
    A0 DOWORK nstid=30 EARLY-RETURN already-running permit=0
doWork() was invoked for a microthread pulled off the work queue (someone scheduleThread'd it) but hit `if (_running) { ...warn...; _rwlock.unlock(); return; }` (thread.cpp ~488). It returns WITHOUT running the microthread AND WITHOUT rescheduling it, and _resumePermit was already consumed (permit=0) -> the wake is SILENTLY DROPPED -> that microthread never runs again -> deadlock (guest: work>0, both workers parked, no signals — exactly the -DNCONS=1 -DNO_STORM repro).

MECHANISM (the race): a microthread can be BOTH still-marked-_running (mid-transition through suspend()/doneWorking — _running is only cleared at the doneWorking tail, thread.cpp ~629) AND have a fresh work item queued by a waker. Even with DSERVER_SINGLE_THREADED, the WorkQueue keeps 1 PERMANENT worker but push() spawns TEMPORARY workers up to hardware_concurrency when _threadsAvailable==0 (workers.hpp:186) -> multiple OS threads drain the queue concurrently. The queued dispatch races the still-_running microthread: doWork() pops it, sees _running, bails without rescheduling. The reverse race (wake-before-suspend) is handled by _resumePermit; the "scheduled-while-still-_running -> doWork sees _running -> DROP" case has NO recovery.

FIX DIRECTION (gate boot FIRST): doWork()'s `if (_running)` branch must NOT silently drop the dispatch. Set a pending-reschedule marker under _rwlock so the doneWorking tail (which already re-checks _resumePermit && _suspended to reschedule, thread.cpp ~634) reschedules once _running clears; OR make the run-loop re-enqueue on this collision. Invariant: "a scheduleThread that lands while _running causes exactly one more doWork after _running clears." Keep minimal; gate: boot smoke -> cvstorm2 -DNCONS=1 -DNO_STORM + all variants RESULT=OK 0/N hang -> brew reinstall A/B 0 hang. bd7cdb9/5766b1d target unrelated layers; 65038e3 (uninit mutex_link) stays. Prod baseline 835946f9, doctor GREEN. Diagnostic traces on branch; repro tmp/cvstorm2.c, harness tmp/a0_cvtrace.sh.

## UPDATE 23 — FIX PART 1 LANDED (signal-free lost wake fixed & gated); a SECOND, storm-only bug remains

Implemented the UPDATE 22 fix (darlingserver 57b86c8): Thread::doWork()'s `if (_running)` guard now records `_rerunPending` instead of silently dropping the dispatch, and the doneWorking tail reschedules on it. CRITICAL detail found during bring-up: the `_rerunPending` reschedule must NOT be gated on `_suspended` (the first attempt gated `resumeAfterWorking` on `_suspended` and STILL hung — trace showed `DONEWORK nstid=30 rerunPending=1 susp=0 -> resched=0`, i.e. the dropped dispatch was for a pending CALL, not a suspended context). Corrected gate: `((_resumePermit && _suspended) || _rerunPending) && !_terminating && !_dead`.

GATE RESULTS (build b58c6c90, boot smoke PASS):
  cvstorm2 -DNCONS=1 -DNO_STORM  => RESULT=OK   (baseline: HANG)   ✅ fixed
  cvstorm2 -DNO_STORM (8 cons)   => RESULT=OK   (baseline: HANG)   ✅ fixed
  cvstorm2 -DNCONS=1 (storm)     => RESULT=HANG                    ❌ still hangs
